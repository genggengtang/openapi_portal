    ...
    window.onload = function () {
        ...
        $("#submit").click(function () {
           ...
           // var cardTypeID= GetQueryString("cardTypeID")
            ...
            // 向后台发送处理数据
            $.ajax({
                url:"${pageContext.request.contextPath}"+api_host+"member/authLogin",
                ...
                success: function (data) {
                    console.log(data)
                    if (data.code =="000" && (typeof(data.data.cardID)!="undefined")) {
                        //回调接口
...
//                            "token"+data.data.token+
                        ...
                        var params=window.location.href.substring(window.location.href.indexOf("?"),window.location.href.length);
                        var url="${pageContext.request.contextPath}/member/register.jsp"+params;
                        $("#telephonenameTip").html("<font color='red'>"+data.msg+"</font><br/>" +
                            "<a href="+url+">注册为会员</a>");
                        return;
                    }
                }
